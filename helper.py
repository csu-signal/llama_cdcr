...
def forward_ab(parallel_model, ab_dict, device, indices, lm_only=False): #tokenized tensors without IM samples 
...
def forward_ab_teacher_student(parallel_model, ab_dict, device, indices, lm_only=False, ann_attn_hidden_logits = False): #tokenized tesnors with IM samples and event pairs
    batch_tensor_ab = ab_dict['input_ids'][indices, :]
    batch_am_ab = ab_dict['attention_mask'][indices, :]
    batch_posits_ab = ab_dict['position_ids'][indices, :]
    batch_student_input_ids =  ab_dict['input_ids_im_student'][indices, :]
    batch_student_am =  ab_dict['attention_mask_im_student'][indices, :]
    batch_teacher_input_ids =  ab_dict['input_ids_im_teacher'][indices, :]
    batch_teacher_am =  ab_dict['attention_mask_im_teacher'][indices, :]
    ...
    batch_am_ab.to(device)
    batch_posits_ab.to(device)
    batch_student_input_ids.to(device)
    batch_student_am.to(device)
    batch_teacher_input_ids.to(device)
    batch_teacher_am.to(device)
    concat_reps_teacher_student = (batch_student_input_ids,batch_student_am,batch_teacher_input_ids, batch_teacher_am )
    ...
    if ann_attn_hidden_logits: 
        ...
        return parallel_model(batch_tensor_ab, attention_mask=batch_am_ab, position_ids=batch_posits_ab,
            global_attention_mask=am_g_ab, arg1=arg1_ab, arg2=arg2_ab,concat_reps_teacher_student = concat_reps_teacher_student, lm_only=lm_only,ann_attn_hidden_logits=ann_attn_hidden_logits )
...
def teacher_student_tokenize(student_tokenizer, teacher_tokenizer, mention_pairs,inner_monologue_map, mention_map, m_end, max_sentence_len=1024, text_key='bert_doc', truncate=True, experiment_type = "no_hypothesis"):

    """
    Tokenizes the mentions pairs with the student model and the IM samples (adbudctive hypothesis) with both the student (Longformer/roberta
    and the teacher model (Llama2) for the  various experiment types.
    no_hypothesis: P  (Coref/E1, E2)
    event_independent/linear chain: event pairs are assumed to be dependent on the IM hypothesis independently (P (Coref/E1, E2) or event pairs are independent of each other i.e., (P (coref/H, E1))
     
    with_hypothesis : coreference is modelled with the IM rationales (hypothesis) along with the event pairs. P(coref/H, E1, E2)


    """
    if max_sentence_len is None:

        max_sentence_len = student_tokenizer.model_max_length

    pairwise_bert_instances_ab = []
    pairwise_bert_instances_ba = []
    im_samples_student = []
    im_samples_teacher = []
    student_doc_start = '<doc-s>'
    student_doc_end = '</doc-s>'
    teacher_doct_start = "<s>[INST]"
    teacher_doc_end = "[/INST]"
    ...
    for (m1, m2) in mention_pairs:
        ...
        def make_student_instance(sent_a, sent_b):
            return ' '.join(['<g>', student_doc_start, sent_a, student_doc_end]), \
                   ' '.join([student_doc_start, sent_b, student_doc_end])
        def make_student_instance_im(sent_a):
            return ' '.join(['<g>', student_doc_start, sent_a, student_doc_end])
        def make_teacher_instance(im_sample):
            return ' '.join([ teacher_doct_start, im_sample, teacher_doc_end]) 

        
        im_sample = inner_monologue_map[(m1, m2)]['im_generated_sample'].replace("\n", "")
        # if experiment_type == "paired_hypo:
        #     paired_context = sentence_b + student_doc_start + im_sample + student_doc_end
        #     instance_ab = make_student_instance(sentence_a, paired_context)
        #     pairwise_bert_instances_ab.append(instance_ab)
        # else:
        instance_ab = make_student_instance(sentence_a, sentence_b)
        pairwise_bert_instances_ab.append(instance_ab)

        instance_ba = make_student_instance(sentence_b, sentence_a)
        pairwise_bert_instances_ba.append(instance_ba)

        im_sample_student = make_student_instance_im(im_sample)
        im_sample_teacher = make_teacher_instance(im_sample)
        im_samples_student.append(im_sample_student)
        im_samples_teacher.append(im_sample_teacher)
    ...
    def truncate_with_mentions(input_ids):
        ...
        for input_id in input_ids:
            ...
            in_truncated = in_truncated + [student_tokenizer.pad_token_id] * (max_sentence_len // 2 - len(in_truncated))
            input_ids_truncated.append(in_truncated)

        return torch.LongTensor(input_ids_truncated)

    def ab_tokenized(pair_wise_instances, im_samples_student,im_samples_teacher=None, experiment_type = None):
        instances_a, instances_b = zip(*pair_wise_instances)

        tokenized_a = student_tokenizer(list(instances_a), add_special_tokens=False)
        tokenized_b = student_tokenizer(list(instances_b), add_special_tokens=False)
        ...
        if experiment_type == "no_hypothesis":
            tokenized_ab_ = torch.hstack((tokenized_a, tokenized_b))
            positions_ab = torch.hstack((positions_a, positions_b))
            tokenized_ab_dict = {'input_ids': tokenized_ab_,
                            'attention_mask': (tokenized_ab_ != student_tokenizer.pad_token_id),
                            'position_ids': positions_ab
                            }
        elif experiment_type == "paired_hypo":

            tokenized_hypo = student_tokenizer(im_samples_student,  return_tensors="pt", padding='max_length', max_length=max_sentence_len, truncation=True)
            #tokenized_ab_ = torch.hstack((tokenized_a, tokenized_b))
            hypo_input_ids = tokenized_hypo['input_ids']
            #hypo_pos_ids = tokenized_hypo['input_ids']
            tokenized_ab_ = torch.hstack((tokenized_ab_, hypo_input_ids))
            positions_hypo = torch.arange(hypo_input_ids.shape[-1]).expand(hypo_input_ids.shape)

            #positions_ab = torch.hstack((positions_a, positions_b))
            hypo_pos_ids  = torch.hstack((positions_ab, positions_b))
            hypo_pos_ids = torch.hstack((hypo_pos_ids, positions_hypo))


            tokenized_ab_dict = {'input_ids': tokenized_ab_,
                            'attention_mask': (tokenized_ab_ != student_tokenizer.pad_token_id),
                            'position_ids': hypo_pos_ids
                            }

        elif experiment_type == "event_independent": # seq length here is smaller for the events, so increase it at this function level when calling for with this condition
            e1_toks =  tokenized_a 
            e1_posids =  positions_a
            e2_toks =  tokenized_b
            e2_posids =  positions_b

            im_tokenized_student = student_tokenizer(im_samples_student,  return_tensors="pt", padding='max_length', max_length=max_sentence_len, truncation=True)
            tokenized_ab_dict = {'input_ids_e1': e1_toks,
                             'attention_mask_e1': (e1_toks != student_tokenizer.pad_token_id),
                             'position_ids_e1': e1_posids,
                             'input_ids_e2': e2_toks,
                             'attention_mask_e2': (e2_toks != student_tokenizer.pad_token_id),
                             'e2_posids': e2_posids,

                             'input_ids_im_student': im_tokenized_student['input_ids'],
                             'attention_mask_im_student': im_tokenized_student['attention_mask'],
                            
                             }


        
        elif experiment_type == "with_hypothesis":

            tokenized_ab_ = torch.hstack((tokenized_a, tokenized_b))
            positions_ab = torch.hstack((positions_a, positions_b))
            im_tokenized_student = student_tokenizer(im_samples_student,  return_tensors="pt", padding='max_length', max_length=max_sentence_len, truncation=True)
            im_tokenized_teacher = teacher_tokenizer(im_samples_teacher,  return_tensors="pt", padding='max_length', max_length=max_sentence_len, truncation=True)
            print("seq length right after tokenization studnet", im_tokenized_student['input_ids'].size())
            print("seq length right after tokenization teacher ", im_tokenized_teacher['input_ids'].size())
            tokenized_ab_dict = {'input_ids': tokenized_ab_,
                             'attention_mask': (tokenized_ab_ != student_tokenizer.pad_token_id),
                             'position_ids': positions_ab,
                             'input_ids_im_student': im_tokenized_student['input_ids'],
                             'attention_mask_im_student': im_tokenized_student['attention_mask'],
                             'input_ids_im_teacher': im_tokenized_teacher['input_ids'],
                             'attention_mask_im_teacher': im_tokenized_teacher['attention_mask'],
                             }

        return tokenized_ab_dict

    if truncate and experiment_type == "no_hypothesis":
        tokenized_ab  = ab_tokenized(pairwise_bert_instances_ab, im_samples_student,im_samples_teacher, experiment_type = experiment_type)
        tokenized_ba = ab_tokenized(pairwise_bert_instances_ba,im_samples_student,im_samples_teacher, experiment_type = experiment_type)
        return tokenized_ab, tokenized_ba
    elif truncate and experiment_type == "paired_hypo":
        tokenized_ab  = ab_tokenized(pairwise_bert_instances_ab, im_samples_student, experiment_type = experiment_type)
         
        return tokenized_ab



    elif truncate and experiment_type == "event_independent":
        tokenized_ab  = ab_tokenized(pairwise_bert_instances_ab,im_samples_student,im_samples_teacher, experiment_type = experiment_type)
        return tokenized_ab

        #tokenized_ba = ab_tokenized(pairwise_bert_instances_ba)
    elif truncate and experiment_type == "with_hypothesis":
        tokenized_ab  = ab_tokenized(pairwise_bert_instances_ab,im_samples_student,im_samples_teacher, experiment_type = experiment_type) 
        return tokenized_ab
        #tokenized_ba = ab_tokenized(pairwise_bert_instances_ba, type = type)
...
def tokenize_with_hypo(tokenizer, mention_pairs,inner_monologue_map, mention_map, m_end, max_sentence_len=1024, text_key='bert_doc', truncate=True ):
    if max_sentence_len is None:
   ...
   # print("mention pair length" ,len(mention_pairs))
    ...
    def ab_tokenized(pair_wise_instances):
        ...
        tokenized_hypo = tokenizer(im_samples_student,  return_tensors="pt", padding='max_length', max_length=max_sentence_len, truncation=True)
        #tokenized_ab_ = torch.hstack((tokenized_a, tokenized_b))
        hypo_input_ids = tokenized_hypo['input_ids']
        #hypo_pos_ids = tokenized_hypo['input_ids']
        #tokenized_ab_ = torch.hstack((tokenized_ab_, hypo_input_ids))
